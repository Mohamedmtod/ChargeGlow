...
actor ChargingActivityManager {
    ...
    func recover() async -> ActivityRecoveryResult {
    ...
    func start(snapshot: BatterySnapshot) async throws -> String {
        _ = await recover()
        ...
        do {
            ...
            await record(error: error, operation: "start")
            throw error
        } catch {
            await record(error: .activityStartFailed, operation: "start")
    ...
    func update(snapshot: BatterySnapshot) async throws {
    ...
    func endAll(snapshot: BatterySnapshot? = nil) async -> ActivityEndResult {
        let activities = ChargingActivity.activities
    ...
    private func record(error: ChargingActivityError, operation: String) async {
        await DiagnosticsRecorder.shared.record(
            category: "activity",
            level: .error,
